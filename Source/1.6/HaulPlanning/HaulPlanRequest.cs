...
namespace UniqueWeaponsUnbound.HaulPlanning
{
    ...
    public class HaulPlanRequest
    {
        ...
        // already passed reach/forbidden/CanReserve gating; the planner is free
        // to choose any subset that meets demand.
        public Dictionary<ThingDef, List<HaulCandidate>> Pool;
    }

    // A single on-map stack the planner may draw from. Position and mass are
    // snapshotted at request-build time so the planner doesn't touch live world
    // state.
    public struct HaulCandidate
    {
        ...
        // building) this stack sits in, snapshotted at pool-build time. Stacks
        // in the same group share an id (>= 0); stacks outside storage get -1
        // and are treated by group-aware planners as singleton groups. Ids
        // index the request's own group table — they are not stable across
        // requests. Only populated when the active planner sets
